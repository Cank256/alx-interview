...
def canUnlockAll(boxes):
    ...
    :param boxes: a list of lists, where each sublist represents a box and contains keys to other boxes
    :return: True if all boxes can be opened, else False
    """
    # The set of unlocked boxes, starting with box 0 since it is initially unlocked
    unlocked = {0}
    ...
    while keys:
        ...
        for key in keys:
            ...
            if 0 <= key < len(boxes) and key not in unlocked:
                ...
                new_keys.extend(boxes[key])  # Add the keys from the newly unlocked box

        # Update the list of keys with the newly found keys
        keys = new_keys

    # Check if we've unlocked all boxes (compare the number of unlocked boxes with the total number of boxes)
